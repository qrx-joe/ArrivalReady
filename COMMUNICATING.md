...
- **结果**：
  ...
  - **演示视频**：docs/demo/arrivalready_demo.mp4（约 148s，1080p，中文配音 + 字幕）；分镜、现场动线、兜底事项与再生成方法见 docs/demo/DEMO_SCRIPT.md。
  - **PO 反馈修订（同日）**：首版解说词偏专业（哈希指纹/落库/IRRS/run/跨租户等），PO 要求改大白话。已按 [PRD 大白话解读](docs/PRD_大白话解读_v0.1.md)的「给店铺做体检」比喻重写全部旁白与第 9 幕工程卡片文案（分数机器算 AI 说了不算 / 结果可复现 / 中断接着跑 / 数据各归各家），重新配音合成（147.6s）并抽帧复检；界面术语（UNKNOWN 等）保留但口播解释。
...
| D-016 | 2026-09-07 | **模型供应商选定阶跃星辰（StepFun）**：PO 直接确认；开放平台 API key 待提供（本机仅有已过期的桌面 Web 会话凭据，不可用于 API 调用）；首次真实调用按方案 §6 预算 ≤6 次 | ✅ 已定（PO 确认） | supersedes D-007（豆包主+Qwen 备的暂定方案作废，历史行保留） |
| D-017 | 2026-09-07 | **比赛交付 deadline = 今日 24:00**（PO 确认）；按方案 §8 时间盒，交付目标诚实降级为技术切片（Slice 1–3），不声称完整比赛 MVP；PRD P0 完成状态以交付声明为准 | ✅ 已定（PO 确认） | supersedes D-006 |
| D-018 | 2026-09-07 | **ADR-0002 评分与人审语义按草案生效**：WARN=0.5、NA 需理由排除分母、UNKNOWN 不计分压覆盖率、Reject 不转 PASS、全覆盖才出总分否则部分评估、S0/S1 独立清单 | ✅ 已定（PO 确认） | supersedes ADR-0002「提议」状态；B10 依此实现 |
