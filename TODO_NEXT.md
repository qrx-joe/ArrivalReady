...
> **2026-09-07 审查补充**：队列与 [执行方案](docs/05_EXECUTION_PLAN_v0.1.md) §9 对照使用。N-03 的最小 Eval runner 应提前至首个模型批次，N-04 安全基础应提前至真实素材入系统前；URL 的 P0/P1 冲突由 B01 明确处理。以下为原队列，尚未视为已完成或已正式调整范围。
...
- URL 证据抓取（**含 SSRF 防护全部要求**，TECH_SPEC §14——做不全就别做这个功能）；
- 报告导出（PDF）；
- 英 / 日目标访客 Profile；
- 团队成员与任务负责人（多用户协作）；
